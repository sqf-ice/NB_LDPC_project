 ...
 *	applications hacked to be a stand alone hash table.
 *
 ...
 *
 * RCS: @(#) $Id: tclHash.c,v 1.1 2004/01/24 23:59:49 emin Exp $
 */
...
#define Tcl_Panic(x) panic(x)
void ckfree(void*v) {free(v);}
void* ckalloc(const size_t size) {return malloc(size);}
...
Tcl_NextHashEntry(register Tcl_HashSearch *searchPtr)
    	/* searchPtr is a place to store information about
